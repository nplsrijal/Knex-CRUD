...
knex('contacts').insert({ firstname: fname, lastname: lname ,email: email, number: num  })
...
app.put('/editcontact',function(req,res,next){
  ...
  knex('contacts').where('id',id).update({ firstname: fname, lastname: lname ,email: email, number: num  })
      .then( function (result) {
          res.json({ success: true, message: 'ok' });     // respond back to request
       })
  ...
  knex('contacts').where('id',id).delete()
      .then( function () {
          res.json({ success: true, message: 'ok' });     // respond back to request
       })
